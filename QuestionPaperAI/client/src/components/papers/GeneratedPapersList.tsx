...
export default function GeneratedPapersList({ paperId, paperTitle, onDelete }: Props) {
  ...
  const { data: generatedPapers, isLoading } = useQuery({
    ...
    queryFn: () => apiRequest('GET', `/api/papers/${paperId}/generated-papers`).then(res => res.json()),
  });

  // Filtering papers by search term (roll number or name)
  const filteredPapers = generatedPapers?.filter((paper: any) => {
    const name = paper.student?.name?.toLowerCase() || "";
    const roll = paper.student?.rollNumber?.toLowerCase() || "";
    return (
  ...
  const deleteMutation = useMutation({
    mutationFn: (id: number) => apiRequest('DELETE', `/api/generated-papers/${id}`),
    onSuccess: () => {
  ...
  const deleteAllMutation = useMutation({
    mutationFn: () => apiRequest('DELETE', `/api/papers/${paperId}/generated-papers`),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['/api/papers', paperId, 'generated-papers'] });
      toast({
  ...
  const handleDownloadDOCX = (paper: any) => {
    ...
    try {
      // Format student info
      const studentName = paper.student?.name?.replace(/\s+/g, '_') || 
                         paper.content.paperHeader?.studentName?.replace(/\s+/g, '_') || 
                         'student';
      const subject = paper.content.paperHeader?.subject?.replace(/[^a-zA-Z0-9]+/g, '_') || 
                     'question_paper';
      const fileName = `${subject}_${studentName}`;

      // Create filtered HTML content
      const paperContent = {
        ...paper.content,
        paperHeader: {
          ...paper.content.paperHeader,
          studentName: paper.student?.name || paper.content.paperHeader?.studentName,
          rollNumber: paper.student?.rollNumber || paper.content.paperHeader?.rollNumber,
          class_section: paper.student?.classSection || paper.content.paperHeader?.class_section
        }
      };

      const htmlContent = getPaperHTMLContent(paperContent);
      ...
      {isLoading ? (
        ...
        <>
          ...
          <div className="mb-4 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
            <input
              className="border rounded px-3 py-2 w-full sm:w-80 focus:outline-none focus:ring-2 focus:ring-primary"
              type="text"
              ...
              <TableBody>
                {filteredPapers.map((paper: any) => (
                  <TableRow key={paper.id}>
                    ...
                    <TableCell>{formatDate(paper.generatedAt) || 'N/A'}</TableCell>
                    <TableCell>
                    ...
                    <TableCell className="text-right space-x-1">
                      ...
                      {/* Modal/inline answer key preview button remains for quick preview */}
                      <Button
                        variant="outline"
                        size="sm"
                      ...
                      {/* Separate link to full answer key page */}
                      <Link href={`/papers/${paperId}/answer-key`}>
                        <Button
                          variant="ghost"
                          size="sm"
                          title="Open Answer Key in New Page"
                        >
