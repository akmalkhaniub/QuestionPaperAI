// New implementation to replace the problematic one
...
async function processGeneratedPapers(paper: any, topics: any[], generatedPapers: any[]) {
  ...
  for (const generatedPaper of generatedPapers) {
    try {
      ...
      // Using question bank approach - this replaces the inner try-catch block
      ...
      try {
        ...
        // generatedContent will remain undefined
      }
      ...
      const filePath = `generated_papers/${paper.id}_${student.rollNumber}_${Date.now()}.pdf`;
